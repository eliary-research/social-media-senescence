...
We formalize this as: the probability that a young user (age 18–29) leaves a platform increases as the proportion of users aged 50+ increases, with a threshold effect when the older-generation adoption rate crosses approximately 40–50% — a parameter we treat as estimated from case data rather than theory-derived (the value matches the Facebook 2016–2018 inflection observed in Pew Research longitudinal data; Perrin & Anderson, 2016, 2018) and is consistent with boyd's framework on youth identity-space dynamics (boyd, 2014). The exact threshold is parameter-fit; §5.2 reports the empirical fit and sensitivity range.
